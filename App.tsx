  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-600 to-indigo-700 text-white flex flex-col">
      <div className="p-4 flex justify-between items-center bg-white/10 backdrop-blur-md sticky top-0 z-10">
        ...
        <div className="font-mono font-bold text-xl flex items-center gap-2"><Flame className="animate-pulse text-blue-200" /> {formatTime(seconds)}</div>
      ...
      <div className="p-4 bg-white/10 backdrop-blur-md sticky bottom-0">
        <button onClick={() => onComplete(seconds)} className="w-full bg-white text-blue-600 font-black py-4 rounded-xl shadow-lg hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2 uppercase tracking-wide text-lg"><Dumbbell size={24} /> Training Starten</button>
      </div>
    </div>
  );
};
...
const PromptModal = ({ onClose, title, icon: Icon, colorClass, currentPrompt, onSave, appendEquipment, equipment, appendHistory, history, description }: any) => {
  ...
  useEffect(() => {
    ...
    if (appendHistory && history) {
      const recentHistoryEntries = history.slice(0, 10).map((h: any) =>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden animate-in zoom-in-95">
        <div className="bg-blue-600 p-4 text-white flex justify-between items-center"><h2 className="font-bold text-lg flex items-center gap-2"><ClipboardCheck /> Plan importieren</h2><button onClick={onClose}><X /></button></div>
        <div className="p-4"><p className="text-xs text-gray-500 mb-2">Füge hier den JSON-Code von ChatGPT ein:</p><textarea value={text} onChange={(e) => setText(e.target.value)} className="w-full h-40 p-3 border border-gray-200 rounded-xl text-xs font-mono bg-gray-50 outline-none resize-none" placeholder='[ { "id": 1, "title": "Push A", ... } ]'></textarea><button onClick={handleImport} className="w-full mt-4 bg-blue-600 text-white font-bold py-3 rounded-xl shadow-md">Plan laden</button></div>
      </div>
    </div>
  );
};
        ...
        <div className="p-4 overflow-y-auto flex-1 space-y-4">
          ...
          <div className="bg-blue-50 p-3 rounded-xl border border-blue-100 flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                <select onChange={handleLoadTemplate} className="flex-1 text-sm border border-blue-200 rounded-lg p-2 outline-none bg-white text-blue-900">
                    <option value="">-- Wähle eine Vorlage --</option>
                    {templates.map((t:any) => <option key={t.id} value={t.id}>{t.title}</option>)}
                </select>
                <button onClick={handleSaveCurrentAsTemplate} className="bg-white border border-blue-200 text-blue-600 p-2 rounded-lg hover:bg-blue-100" title="Aktuelles als Vorlage speichern"><Save size={18}/></button>
              </div>
          </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <input type="text" placeholder="z.B. Brust & Trizeps" value={title} onChange={e => setTitle(e.target.value)} className="w-full border border-gray-200 rounded-xl p-2 font-bold text-gray-900 outline-none focus:border-blue-500"/>
            </div>
             <div>
              <label className="text-[10px] font-bold text-gray-400 uppercase">Fokus / Muskelgruppe</label>
              <input type="text" placeholder="z.B. Oberkörper" value={focus} onChange={e => setFocus(e.target.value)} className="w-full border border-gray-200 rounded-xl p-2 font-bold text-gray-900 outline-none focus:border-blue-500"/>
            </div>
            <div>
          ...
          <button onClick={addExercise} className="w-full py-3 border-2 border-dashed border-gray-300 rounded-xl text-gray-400 font-bold hover:bg-gray-50 hover:border-blue-300 hover:text-blue-500 transition-all flex items-center justify-center gap-2">
            <PlusCircle size={18}/> Weitere Übung
          </button>
        </div>

        <div className="p-4 border-t border-gray-100 shrink-0">
          <button onClick={handleSave} className="w-full bg-blue-600 text-white font-bold py-3 rounded-xl shadow-lg active:scale-95 transition-transform">
            Plan Speichern
          </button>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white rounded-3xl w-full max-w-xs shadow-2xl p-6 animate-in zoom-in-95">
        <div className="flex flex-col items-center text-center mb-6"><div className="w-12 h-12 bg-blue-100 text-blue-500 rounded-full flex items-center justify-center mb-3"><AlertTriangle size={24} /></div><h3 className="text-lg font-black text-gray-900">Training verlassen?</h3><p className="text-sm text-gray-500 mt-1">Dein Fortschritt wird lokal gespeichert.</p></div>
...
function App() {
  ...
  const handleCSVExport = () => {
    ...
    link.href = url;
    link.download = `you-vs-you-backup-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  const handleExport = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `you-vs-you-backup-${new Date().toISOString().slice(0, 10)}.json`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  ...
  const handleReset = () => {
    if (confirm("Alles zurücksetzen? Alle Daten und der Verlauf werden gelöscht.")) {
      ...
      setCooldownPrompt(DEFAULT_COOLDOWN_PLAN);
  ...
  return (
    <div className="min-h-screen bg-neutral-900 flex justify-center font-sans">
      <div className="w-full max-w-md bg-gray-50 min-h-screen relative shadow-2xl overflow-hidden">
        ...
        {activePromptModal === 'warmup' && <PromptModal onClose={() => setActivePromptModal(null)} title="Warm-up Prompt" icon={Zap} colorClass="bg-gradient-to-r from-blue-600 to-indigo-600" currentPrompt={warmupPrompt} onSave={handleSaveWarmupPrompt} />}
        {activePromptModal === 'cooldown' && <PromptModal onClose={() => setActivePromptModal(null)} title="Mein Cool Down" icon={Wind} colorClass="bg-gradient-to-r from-blue-500 to-indigo-500" currentPrompt={cooldownPrompt} onSave={handleSaveCooldownPrompt} description="Definiere hier deinen Cool Down Ablauf:" />}
        ...
        {previewWorkout && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
            <div className="bg-white rounded-3xl w-full max-w-sm max-h-[80vh] flex flex-col shadow-2xl animate-in zoom-in-95">
              <div className={`p-5 rounded-t-3xl text-white bg-gradient-to-r from-blue-800 to-blue-900 flex justify-between items-start shrink-0`}>
        ...
        <div className="pb-24 min-h-screen">
            ...
            <>
              <header className="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 pb-6 shadow-lg text-white rounded-b-3xl">
                <div className="mb-6 text-center">
                    <h1 className="text-2xl font-black italic text-white uppercase tracking-widest border-b-2 border-blue-200 pb-2 inline-block shadow-sm">
          ...
          {activeTab === 'history' && !selectedHistoryEntry && (
            <div className="pb-0">
              <header className="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 pb-12 text-white shadow-lg rounded-b-3xl">
                <h1 className="text-2xl font-black">Verlauf</h1>
              </header>
              <div className="p-4 -mt-8 space-y-4">
                {history.map((entry: any) => (
                  <div key={entry.id} onClick={() => setSelectedHistoryEntry(entry)} className="bg-white p-5 rounded-2xl shadow-md border-l-4 border-emerald-500 border-gray-100 flex justify-between items-center cursor-pointer hover:shadow-lg transition-all">
                    <div className="flex-1 pr-4">
                      <h3 className="font-bold text-gray-900 text-lg">{entry.workoutTitle}</h3><p className="text-xs text-gray-500 mt-1">{formatDate(entry.date)}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="fixed bottom-0 w-full max-w-md mx-auto bg-white border-t border-gray-200 px-6 py-2 pb-6 flex justify-between items-center text-xs font-medium z-20 left-0 right-0">
          <button onClick={() => setActiveTab('profile')} className={`flex flex-col items-center gap-1 p-2 rounded-lg flex-1 transition-colors ${activeTab === 'profile' ? 'text-blue-600' : 'text-gray-400 hover:text-gray-600'}`}><UserCircle className="w-6 h-6" /><span>Profil</span></button>
          <button onClick={() => setActiveTab('training')} className={`flex flex-col items-center gap-1 p-2 rounded-lg flex-1 transition-colors ${activeTab === 'training' ? 'text-blue-600' : 'text-gray-400 hover:text-gray-600'}`}><Dumbbell className="w-6 h-6" /><span>Training</span></button>
          <button onClick={() => setActiveTab('history')} className={`flex flex-col items-center gap-1 p-2 rounded-lg flex-1 transition-colors ${activeTab === 'history' ? 'text-blue-600' : 'text-gray-400 hover:text-gray-600'}`}><History className="w-6 h-6" /><span>Verlauf</span></button>
        </div>
      </div>
    </div>
  );
